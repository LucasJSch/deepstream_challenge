...
generate_object_object (void *privData, NvDsEventMsgMeta *meta)
{
  ...
  switch (meta->objType) {
    ...
    case NVDS_OBJECT_TYPE_CUSTOM:
      ...
      if (meta->extMsgSize) {
        NvDsScheinkObject *dsObj = (NvDsScheinkObject *) meta->extMsg;
        if (dsObj) {
          // Iterate the zoneCounts map and add to the json object
          for (auto it = dsObj->zoneCounts.begin(); it != dsObj->zoneCounts.end(); ++it) {
            json_object_set_int_member (jobject, it->first.c_str(), it->second);
          }
        }
      } else {
        // No person object in meta data. Attach empty person sub object.
        json_object_set_int_member (jobject, "No zones registered", 0);
      }
      json_object_set_object_member (objectObj, "zones", jobject);
      break;    
    default:
