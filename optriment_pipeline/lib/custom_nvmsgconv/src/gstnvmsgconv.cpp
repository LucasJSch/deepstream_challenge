...
G_DEFINE_TYPE_WITH_CODE (GstNvMsgConv, gst_nvmsgconv, GST_TYPE_BASE_TRANSFORM,
    GST_DEBUG_CATEGORY_INIT (gst_nvmsgconv_debug_category, "custom_nvmsgconv", 0,
...
gst_nvmsgconv_copy_meta (gpointer data, gpointer uData)
{
  ...
  if (srcPayload) {
    ...
    outPayload->payload =
        g_memdup (srcPayload->payload, srcPayload->payloadSize);
...
{
  return gst_element_register (plugin, "custom_nvmsgconv", GST_RANK_NONE,
      GST_TYPE_NVMSGCONV);
}

#define PACKAGE "custom_nvmsgconv"

GST_PLUGIN_DEFINE (GST_VERSION_MAJOR,
    GST_VERSION_MINOR,
    custom_msgconv,
    "Custom msgconv plugin",
    plugin_init, "7.1", "Proprietary", "CustomNvMsgConv", "http://nvidia.com")
